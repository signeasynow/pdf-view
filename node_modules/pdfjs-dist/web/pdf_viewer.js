...
class PDFPageView {
  ...
  constructor(options) {
    ...
    console.log(options.initialAnnotations, 'options.initialAnnotations2')
    ...
    div.addEventListener('text-click', (event) => {
      this.eventBus.dispatch("edit-original-text-selected", {
        detail: event.detail,
        pageNumber: this.id
      });
  ...
  redrawText() {
    ...
    if (this.textLayer) {
      console.log("rendering bro3")

      // Call the private #renderTextLayer method
      this.#renderTextLayer();
    }
  }
...
class PDFViewer {
  ...
  setDocument(pdfDocument, initialAnnotations) {
    ...
    Promise.all([firstPagePromise, permissionsPromise]).then(([firstPdfPage, permissions]) => {
      ...
      console.log(initialAnnotations, 'initialAnnotations in pdf_viewer');
  ...
  scrollPageIntoView({
    ...
    verticalPercentage = Math.min(Math.max(verticalPercentage, 0), 1);

    const changeOrientation = pageView.rotation % 180 !== 0;
    const pageWidth = (changeOrientation ? pageView.height : pageView.width) / pageView.scale / _pdfjsLib.PixelsPerInch.PDF_TO_CSS_UNITS;
    const pageHeight = (changeOrientation ? pageView.width : pageView.height) / pageView.scale / _pdfjsLib.PixelsPerInch.PDF_TO_CSS_UNITS;

    let y1 = pageHeight * (1 - verticalPercentage); // Invert percentage for Y-axis calculation
    const viewportY = pageView.viewport.convertToViewportPoint(0, y1)[1];
